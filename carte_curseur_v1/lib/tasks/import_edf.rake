...
# rake db:import_edf_data['/home/commun/ecole/REC/PRISM/EUGene/a_importer/','testGuy']
...
namespace :db do
  ...
  task :import_edf_data, :folder, :file, :needs=>:environment do |t, args|  
    ...
    edffilename = args.file
    
      ###################################################
      # ancienne fonction edf_to_yaml(@folder, edffile) #
      ###################################################
        require('yaml')
        f1=File.open(folder+edffilename+".edf","r")
        yamlfile=File.open(folder+edffilename+".yml","w")
        first_variable=false
    
        f1.each do |line|
          if first_variable == false
            if line =~ /variable = "/ 
              yamlfile.write("variables:\n")
              first_variable = true
            end
          end
